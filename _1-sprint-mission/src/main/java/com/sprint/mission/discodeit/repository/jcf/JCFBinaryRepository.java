package com.sprint.mission.discodeit.repository.jcf;

import com.sprint.mission.discodeit.entity.BinaryContent;
import com.sprint.mission.discodeit.repository.BinaryContentRepository;
import org.springframework.stereotype.Repository;

import java.util.*;

@Repository
public class JCFBinaryRepository implements BinaryContentRepository {

  private final Map<UUID, BinaryContent> binaryContentData;

  public JCFBinaryRepository() {
    this.binaryContentData = new HashMap<>();
  }

  @Override
  public BinaryContent save(BinaryContent binaryContent) {
    this.binaryContentData.put(binaryContent.getId(), binaryContent);
    return binaryContent;
  }

  @Override
  public Optional<BinaryContent> findById(UUID binaryContentId) {
    return Optional.ofNullable(this.binaryContentData.get(binaryContentId));
  }

  @Override
  public List<BinaryContent> findAllByIdIn(List<UUID> binaryContentIds) {
    return this.binaryContentData.values().stream()
        .filter(content -> binaryContentIds.contains(content.getId()))
        .toList();
  }

  @Override
  public void deleteById(UUID binaryContentId) {
    this.binaryContentData.remove(binaryContentId);
  }

  @Override
  public boolean existsById(UUID binaryContentId) {
    return this.binaryContentData.containsKey(binaryContentId);
  }
}
